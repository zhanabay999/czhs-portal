...
import { Newspaper, ChevronLeft, ChevronRight, Archive, Clock } from "lucide-react";
import { ArchiveAccordion } from "@/components/ArchiveAccordion";

type Props = {
  params: Promise<{ locale: string }>;
  searchParams: Promise<{ page?: string; category?: string; search?: string; tab?: string }>;
};
...
export default async function NewsPage({ params, searchParams }: Props) {
  ...
  const { page: pageStr, category, search, tab } = await searchParams;
  ...
  try {
    ...
    if (isArchiveTab) {
      archivedArticles = await db
        ...
        .from(newsArticles)
        .where(eq(newsArticles.status, "archived"))
  ...
  // Group archived articles by year → month for ArchiveAccordion
  const archiveByYear: Record<string, { year: number; months: Record<string, { label: string; articles: typeof archivedArticles }> }> = {};
  for (const article of archivedArticles) {
    if (!article.publishedAt) continue;
    const d = new Date(article.publishedAt);
    const year = d.getFullYear();
    const monthKey = `${year}-${String(d.getMonth() + 1).padStart(2, "0")}`;
    const monthLabel = d.toLocaleDateString(isKk ? "kk-KZ" : "ru-RU", { month: "long" });
    const yearStr = String(year);
    if (!archiveByYear[yearStr]) archiveByYear[yearStr] = { year, months: {} };
    if (!archiveByYear[yearStr].months[monthKey]) archiveByYear[yearStr].months[monthKey] = { label: monthLabel, articles: [] };
    archiveByYear[yearStr].months[monthKey].articles.push(article);
  }
  const archiveYearsData = Object.entries(archiveByYear)
    .sort((a, b) => Number(b[0]) - Number(a[0]))
    .map(([, { year, months }]) => ({
      year,
      months: Object.entries(months)
        .sort((a, b) => b[0].localeCompare(a[0]))
        .map(([monthKey, { label, articles }]) => ({ monthKey, label, articles })),
    }));
  ...
  return (
    <NewsContent
      ...
      articles={articles}
      archiveYearsData={archiveYearsData}
...
function NewsContent({
  ...
  archiveYearsData: Array<{ year: number; months: Array<{ monthKey: string; label: string; articles: Array<{ id: string; slug: string; titleKk: string; titleRu: string; excerptKk: string | null; excerptRu: string | null; publishedAt: Date | null }> }> }>;
    ...
    <div className="bg-white">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex gap-1 rounded-lg border border-gray-200 bg-gray-50 p-1 w-fit">
          <Link href="/news">
            ...
            >
              ...
              {isKk ? "Жаңалықтар" : "Актуальные"}
            </button>
          </Link>
          <Link href="/news?tab=archive">
            <button
              className={`flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium transition-colors ${
                isArchiveTab
                  ? "bg-white text-[#003DA5] shadow-sm"
        ...
        {isArchiveTab ? (
          ...
          <div>
            {archiveYearsData.length === 0 ? (
              <div className="rounded-lg border border-dashed border-gray-300 p-12 text-center">
                <Archive className="mx-auto mb-4 h-12 w-12 text-gray-300" />
                <p className="text-gray-500">{isKk ? "Мұрағат бос" : "Архив пуст"}</p>
              </div>
            ) : (
              <ArchiveAccordion years={archiveYearsData} locale={locale} />
            )}
          </div>
        ) : (
          /* News tab */
